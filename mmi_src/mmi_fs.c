...
#define			MMI_DQ_FS_FP_MAX_NUM    	20
...
static void mmi_dq_fds_read(mid_fds_file_id file,  unsigned char* r_data ,unsigned int r_size)
{
	unsigned int i;
#if 0
	switch(file)
...
static RET_VAL mmi_dq_fds_write(mid_fds_file_id file, unsigned char* w_data ,unsigned int w_size)
{
	unsigned int i;
#if 0
	switch(file)
...
{
	if((fp_index < MMI_DQ_FS_FP_MAX_NUM) && (g_dq_fs_fp[fp_index].fp_index == fp_index))
		return RET_SUCESS;
	else
		return RET_FAIL;
}
...
{
	if(mmi_dq_fs_check_fp(fp_index) == RET_SUCESS)
	{
...
{
	return g_dq_fs_init_set.alarm_flag;
}
