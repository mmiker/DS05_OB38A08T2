			...
			else if((key_val == KEY_H) || (status == SYS_STATUS_INPUT_PWD && key_len == KEY_INPUT_MAX_LEN) || (status != SYS_STATUS_INPUT_PWD && key_len == PWD_INPUT_MAX_LEN))
			{
				if(key_len < PWD_INPUT_MIN_LEN)
				{
					if(status == SYS_STATUS_INPUT_PWD)
					{
						...
						else if(key_len == 2 && key_input[0] == BASE_KEY_0 &&key_input[1] == BASE_KEY_1 )
						{
							...
							if (mmi_dq_fs_get_business_flag()==1)
							{
								...
								mmi_dq_fs_set_business_flag(1,NULL);
								mmi_dq_aud_play_with_id(AUD_ID_OUT_OPEN, NULL);
				...
				else
				{
					if(status == SYS_STATUS_INPUT_PWD)
					{
						if(mmi_dq_fs_check_input_pwd_for_open(input_key_1,key_len) == 0)
							mmi_dq_sys_door_open_fail(SYS_OPEN_BY_PASSWORD);
						else
