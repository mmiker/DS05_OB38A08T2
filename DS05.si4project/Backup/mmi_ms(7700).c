...
void mmi_task_proc(void)
{
	...
	touch_value = mmi_dq_rst_get_state();
	if(touch_value != 0)
		mmi_dq_ms_set_msg_que(QUE_EVENT_RST,QUE_PRO_LOW,0);

	key_value = mmi_dq_sys_get_timer2_flag();
	if(key_value != 0)
		mmi_dq_ms_set_msg_que(QUE_EVENT_TIMER_END,QUE_PRO_LOW,key_value);
	...
	mmi_dq_led_task_proc();
	mmi_dq_alarm_task_pro();
	return;
}
...
void mmi_sleep_task_proc(void)
{
	...
	if((mmi_dq_key_check() != 0) || (mmi_dq_rst_get_pin() == 0)
#ifdef __LOCK_FP_SUPPORT__
		||(mmi_dq_fp_get_pin() == 0)
#endif
		)
...
#endif
			case QUE_EVENT_RST:
				mmi_ms_reset_opt_fun(sys_msg_que.que_data);
				break;
	...
	switch(status)
	{
		case SYS_STATUS_IDLE:
			if(key_val == KEY_S)
			{
				...
				mmi_dq_fs_set_alarm_flag(1);
				mmi_dq_aud_play_with_id(AUD_ID_SET_ALARM_SUCESS);
				mmi_dq_inside_led_timer_stop();
		...
		case SYS_STATUS_ADD_110_PWD:
			...
			mmi_dq_aud_play_key_tone();
			if(status == SYS_STATUS_INPUT_PWD)
				mmi_dq_signal_led_shine_timer_start(LED_TYPE_RIGHT, 1, 0);
			else if(status == SYS_STATUS_INPUT_ADMIN_PWD)
				mmi_dq_signal_led_shine_timer_start(LED_TYPE_RIGHT, 1, 1);
			...
			if(key_val == KEY_S)
			{
				if(status == SYS_STATUS_INPUT_PWD)
				{
					...
					{
						mmi_dq_set_aud_state(1);
						if(mmi_dq_fs_get_pro_sound() == 0)
						{
				...
				if(key_len == 0)
				{
					if((status == SYS_STATUS_ADD_PWD)||(status == SYS_STATUS_ADD_110_PWD)||(status == SYS_STATUS_ADD_ADMIN_PWD))
					{
						if(opt_time == OPT_ONE_TIME)
							mmi_dq_aud_play_with_id(AUD_ID_INPUT_NEW_PWD);
						else
							mmi_dq_aud_play_with_id(AUD_ID_PWD_INPUT_AGAIN);
					}
					else if(status == SYS_STATUS_INPUT_PWD)
						mmi_dq_aud_play_with_id(AUD_ID_INPUT_PWD);
					else if(status == SYS_STATUS_INPUT_ADMIN_PWD)
						mmi_dq_aud_play_with_id(AUD_ID_INPUT_ADMIN_PWD);
				}

			}
			else if((key_val == KEY_H) || (status == SYS_STATUS_INPUT_PWD && key_len >= KEY_INPUT_MAX_LEN) || (status != SYS_STATUS_INPUT_PWD && key_len >= PWD_INPUT_MAX_LEN))
			{
				if(key_len < PWD_INPUT_MIN_LEN)
				{
					...
					{
						mmi_dq_signal_led_shine_timer_start(LED_TYPE_ERROR, 2, 0);
						mmi_dq_aud_play_with_id(AUD_ID_INPUT_FAIL_RETRY);
						key_len = 0;
						memset(input_key_1,0xFF,sizeof(input_key_1));
					}
					else
				...
				else
				{
					if(status == SYS_STATUS_INPUT_PWD)
					{
						...
						if(ret == 0xFF)
							mmi_dq_sys_door_open_fail(SYS_OPEN_BY_PASSWORD);
#ifdef __LOCK_BUS_SUPPORT__
						else if(ret == 4 && mmi_dq_fs_get_business_flag()==1)
						{
					...
					else if((status == SYS_STATUS_ADD_PWD)||(status == SYS_STATUS_ADD_110_PWD)||(status == SYS_STATUS_ADD_ADMIN_PWD))
					{
						...
						else if(opt_time == OPT_TWO_TIME)
						{
							if(0 == memcmp(input_key_1,input_key_2,PWD_INPUT_MAX_LEN))
							{
								if(status == SYS_STATUS_ADD_PWD)
								{
									if(mmi_dq_fs_set_pwd(input_key_1,key_len,FDS_USE_TYPE_USER) == RET_FAIL)
										mmi_dq_aud_play_with_id(AUD_ID_CHG_FAIL);
									else
										mmi_dq_aud_play_with_id(AUD_ID_CHG_PWD_SUCESS);
									mmi_dq_sys_show_cur_menu_list();
								}
							...
							{
								mmi_dq_aud_play_with_id(AUD_ID_PWD_NOT_SAME);
							}
							mmi_ms_pwd_init_var();
						}
					}
			...
			break;
#if defined (__LOCK_FP_SUPPORT__) || defined(__LOCK_RFID_CARD_SUPPORT__)
#ifdef __LOCK_FP_SUPPORT__
...
void mmi_ms_fps_opt_fun(unsigned char fps_val)
{
	...
	if((status != SYS_STATUS_IDLE && status != SYS_STATUS_INPUT_FP && status != SYS_STATUS_ADD_FP && status != SYS_STATUS_ADD_110_FP && status != SYS_STATUS_DEL_FP)
#ifdef 	__FACTORY_TEST_SUPPORT__
	...
	if(retval == 0)
	{
		...
		if(retval == 0)
		{
			...
			if(retval == 0)
			{
				if(status == SYS_STATUS_INPUT_FP || status == SYS_STATUS_IDLE)
				{
					retval = mmi_dq_fs_check_fp((unsigned char)index);
				...
				else if(status == SYS_STATUS_ADD_FP || status == SYS_STATUS_ADD_110_FP || status == SYS_STATUS_FM_MODE)
				{
					mmi_dq_fp_light(FP_RED);
				...
				else if(status == SYS_STATUS_DEL_FP)
				{
					...
					if(retval == RET_SUCESS)
					{
						...
						else
						{
							...
							{
								retval = mmi_dq_fs_del_fp((unsigned char)index);
								if(retval == RET_SUCESS)
								{
				...
				else if(status == SYS_STATUS_ADD_FP || status == SYS_STATUS_ADD_110_FP || status == SYS_STATUS_FM_MODE)
				{
					if(opt_time == FPS_MAX_INPUT_TIME)
					{
						...
						if(retval == 0)
						{
							...
							{
								index = 0;
								//if(mmi_dq_fs_check_fp(index) == RET_SUCESS)
							...
							else
							{
								...
								if(status == SYS_STATUS_ADD_FP || status == SYS_STATUS_ADD_110_FP)
								{
									if(retval == 0)
									{
										retval = mmi_dq_fs_set_fp((unsigned char)index);
										if(retval != 0)
											mmi_dq_fp_delete(index);
									}
									if(retval == 0)
...
return :
	none
*/
void mmi_ms_reset_opt_fun(unsigned char reset_val)
{
	SYS_BASE_STATUS status = mmi_dq_ms_get_sys_state();
	...
	else if(reset_val == 1)//long press
	{
#ifdef 	__FACTORY_TEST_SUPPORT__
