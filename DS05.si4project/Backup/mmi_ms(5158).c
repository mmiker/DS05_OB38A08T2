	...
	switch(status)
	{
		...
		case SYS_STATUS_ADD_ADMIN_PWD:
			...
			if((key_val >= KEY_0 && key_val <= KEY_9) && key_len < KEY_INPUT_MAX_LEN)
			{
				...
				else if((status == SYS_STATUS_ADD_PWD)||(status == SYS_STATUS_ADD_ADMIN_PWD))
				{
			...
			if(key_val == KEY_S)
			{
				...
				if(key_len == 0)
				{
					if((status == SYS_STATUS_ADD_PWD)||(status == SYS_STATUS_ADD_ADMIN_PWD))
					{
						if(opt_time == OPT_ONE_TIME)
			...
			else if((key_val == KEY_H) || (status == SYS_STATUS_INPUT_PWD && key_len >= KEY_INPUT_MAX_LEN) || (status != SYS_STATUS_INPUT_PWD && key_len >= PWD_INPUT_MAX_LEN))
			{
				...
				else
				{
					if(status == SYS_STATUS_INPUT_PWD)
					{
						...
						{
#ifdef __LOCK_BUS_SUPPORT__
							if(ret == 4 && mmi_dq_fs_get_business_flag()==1)
							{
...
void mmi_ms_fps_opt_fun(unsigned char fps_val)
{
	...
	if((status != SYS_STATUS_IDLE && status != SYS_STATUS_INPUT_FP && status != SYS_STATUS_ADD_FP && status != SYS_STATUS_DEL_FP)||(status == SYS_STATUS_FM_MODE && STR_ID_FINGERPRINT != mmi_dq_factory_mode_get_test_project()))
		return;
	...
	if(retval == 0)
	{
		...
		if(retval == 0)
		{
			...
			if(retval == 0)
			{
				...
				else if(status == SYS_STATUS_ADD_FP || status == SYS_STATUS_FM_MODE)
				{
					mmi_dq_fp_light(FP_RED);
