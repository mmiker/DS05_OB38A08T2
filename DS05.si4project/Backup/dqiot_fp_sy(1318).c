...
/**
  ...
  * @see    none
  */
static unsigned char JudgeStr(unsigned int waittime, unsigned char length)
{
...
unsigned char PS_Search(unsigned char BufferID,unsigned short StartPage,unsigned short PageNum,void *p)
{
	...
	sure = SendCheck(0x01+0x08+0x04+BufferID+(StartPage>>8)+(unsigned char)StartPage+(PageNum>>8)+(unsigned char)PageNum);
